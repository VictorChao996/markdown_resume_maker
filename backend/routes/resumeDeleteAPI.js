...
router.delete("/", async (req, res) => {
    ...
    if (!resumeIdList.includes(resumeId)) {
        res.status(400).send(
            "Invalid resume id for this user: either the resumeId is not exist or the resumeId is not belong to this user"
        )
        return
    }

    const resume = await Query.deleteResume(resumeId)
    if (!resume) {
        res.status(500).send("Internal Server Error: Failed to get resume")
        return
    }
    res.status(200).json({
        "Successfully deleted resume": resumeId
    })
})
